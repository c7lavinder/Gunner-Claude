...
| 2026-05-04 | `20260504000000_add_agent_wholesaler` | **Session 67 Phase 1** — purely additive. 4 new tables: `agents`, `wholesalers`, `property_agents` (composite PK propertyId+agentId), `property_wholesalers` (composite PK propertyId+wholesalerId). 4 indexes, 6 foreign keys. Mirrors v1.1 Wave 4-5 Seller/Buyer first-class-entity pattern. No app code reads them yet — Phase 2 (property-detail link UX) ships separately. Plan: `~/.claude/plans/at-te-he-very-base-mellow-pixel.md`. |
