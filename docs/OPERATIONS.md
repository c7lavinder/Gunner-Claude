...
- `embed-calls-backfill.ts` — **Session 82.** Reads every `calls` row where `transcript_embedding IS NULL AND transcript IS NOT NULL AND grading_status = 'COMPLETED'` and writes a 1536-dim vector built from contact + type + outcome + aiSummary + transcript (6500-char cap). Idempotent (only fills nulls). Cost: ~$0.00002 per call (5K calls ≈ $0.10). Supports `--dry-run`, `--tenant=<tenantId>`, `--limit=N`. Required: `OPENAI_API_KEY`. Run AFTER applying migration `20260511000000_session_summary_and_call_embeddings`. Powers `lib/ai/query-tools.ts → semanticSearchCalls`.
- `backfill-lost-opps.ts` — **Session 81.** Walks every active `tenant_ghl_pipelines` row via `ghl.searchOpportunities` (cursor-paginated, up to 100 pages × 100 opps = 10k cap per pipeline). For each opp where `status` is `lost` / `abandoned`, finds the matching `Property` by `ghl{Acq,Dispo,Longterm}OppId` and stamps `acqLostAt` / `dispoLostAt` / `longtermLostAt`. Also clears `*LostAt` for any property whose opp is back to `open` / `won` (covers reopens that happened before the webhook in `lib/ghl/webhooks.ts` listened for `status`). Idempotent — skips already-stamped rows. `--dry-run` + `--tenant=<slug>` supported. Ran live on `new-again-houses` 2026-05-11: **34 stamped** (1 acq / 32 dispo / 1 longterm) across 8,658 opps; 271 Lost opps had no matching Property (correctly skipped — they were Lost before becoming Gunner properties). Re-run when chip counts drift from GHL truth or after a long webhook outage.
