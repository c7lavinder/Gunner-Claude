// app/api/diagnostics/embed-calls-backfill/route.ts
//
...
// to ship the key to a local terminal. Lives under /api/diagnostics so the
// middleware's PUBLIC_PATHS allow-list lets the bearer auth fire instead
// of redirecting to /login.
//
// Auth: same pattern as the rest of app/api/diagnostics/*. Bearer
// DIAGNOSTIC_TOKEN. Fails closed (401) when the token env var is unset.
//
// Usage:
//   curl -X POST \
//     -H "Authorization: Bearer ${DIAGNOSTIC_TOKEN}" \
//     "<PRODUCTION_URL>/api/diagnostics/embed-calls-backfill?limit=200"
//
