...
Spot-check the Session 80 changes on a recently-created property:

1. Open the property detail page on a property created in the last
   2-3 weeks (PR enrichment will have fired).
2. Look at the **Property Details panel** — should now show a **third
   row** with Mortgage & Equity / Distress / MLS & Records columns
   (each marked with a purple "PR" tag). Empty columns auto-hide;
   distress flags that are TRUE highlight amber. If row is missing,
   that property had no vendor enrichment data — click Re-enrich.
3. Look at the **Deal Story** card on the Overview tab. Should read
   like English: "Acquisition: Under contract" not "DISPO_NEW", and
   "Buyer showed up" not "Showed". Hit Regenerate if it's stale.
4. Run a dispo blast generation on a property in disposition (Section 2
   of the Disposition Journey → Generate description / listing / FB
   post). Verify no fabricated dollar amounts — every number in the
   output should match a number visible somewhere on the property page.

If any of these break the rules in
[feedback_ai_strict_facts.md](~/.claude/projects/-Users-coreylavinder-Downloads-gunner-ai-GOOD/memory/feedback_ai_strict_facts.md)
(memory) or look wrong visually, flag in next session.

**Owner action items still carried forward (no time pressure):**
