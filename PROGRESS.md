...
**Pass 13 — Phase 10 PRODUCT (typed calibration UI + corrected mine source).**
User: "lets do it" — start Phase 10 product. Found two interlocking bugs:
(a) the call-detail Flag button toggled `Call.isCalibration` but never
wrote a kind (good/bad) or notes, so even when used the data was
useless as an eval seed; (b) `mine-eval-candidates.ts` was querying
`AiLog.status IN ('rejected','edited')` — that enum exists in schema
but no production code writes it, because the assistant execute route
persists reject/edit to `action_logs.wasRejected/wasEdited` instead.

Three fixes shipped + tested end-to-end against prod:
1. `app/api/[tenant]/calls/[id]/calibration/route.ts` — new contract:
   `POST {kind: 'good'|'bad', notes?: string}` (mark) or
   `{isCalibration: false}` (clear). Persists as
   `calibrationNotes = "<kind>: <notes>"` matching the assistant
   execute-route convention.
2. `components/calls/call-detail-client.tsx` — replaced Flag button with
   a Good/Bad/Clear popover. ThumbsUp + ThumbsDown icons, optional 500-
   char notes textarea, semantic-green/red tinting. Parses `<kind>:`
   prefix from existing notes so legacy rows render as un-typed.
3. `scripts/mine-eval-candidates.ts` — Section 1 rewritten to query
   `action_logs WHERE was_rejected OR was_edited` (the actually-
   populated source). Section 3 now buckets calibration entries by
   👍 Good / 👎 Bad / Un-typed by parsing the prefix.

Smoketested: wrote `"good: phase 10 smoketest ..."` to a real call's
calibrationNotes via direct Prisma update, read it back, restored
original state. Confirmed the contract round-trips.

Commit `a5a230e4`. Pushed `93b84035..a5a230e4`.

**Pass 14 — QA-pass gap fix (legacy calibration payload).**
User: "is everything pushed? any gaps?" — final QA sweep before wrap.

Found 1 real gap + 1 milestone:
- **Gap (FIXED):** Pass-13 commit's calibration route docstring claimed
  backwards-compat for the legacy `{isCalibration: true}` payload —
  but the code returned 400 because `kindNormalized` was null and the
  kind check ran before any legacy branch. No active caller hit this
  path (new UI sends typed shape only), but the docstring lied. Fix:
  route now resolves three payload shapes (typed / clear / legacy)
  explicitly before the 400 gate. Legacy path persists
  `isCalibration=true, calibrationNotes=null` and logs an auditLog
  payload with `legacy: true` so future analytics can identify + re-
  prompt managers to re-classify.
- **Milestone (no action):** drift-report ran against last-24h prod
  data and **showed real telemetry for the first time** —
  10/103 rows versioned across 4 surfaces × 3 versions:
  `call_grading 1.1.0`, `deal_intel 1.4.0`, `next_steps 1.0.0`,
  `property_story 1.1.0`. Phase 8 payoff working in production.
- Other gap-audit findings: residual smoketest data (0 leftover ✓),
  Tailwind semantic classes (defined ✓), orphan imports (none ✓),
  schema sync (clean ✓).

Commit `d8a2517b`. Pushed `a5a230e4..d8a2517b`.

**Session 89 grand total: 14 keep-going passes across one calendar day,
7 commits on main, 36 files changed/created, end-to-end LLM Rewiring
Plan ship.** No commits pending. No working-tree changes. Drift signal
live. Phase 10 product loop functional and end-to-end smoketested.
...
**All 10 phases of the LLM Rewiring Plan shipped end-to-end (Sessions 87-89).
Phase 8 drift signal is producing live telemetry in prod: 10/103 ai_log
rows in the last 24h carry `prompt_version`. The next concrete work is
USAGE-DRIVEN, not new infra.**

### Start here in Session 90

**Step 1 — confirm the Phase 10 calibration loop is being exercised.**

Run the mine script:
```bash
npx tsx scripts/mine-eval-candidates.ts
```

What to look for: rising counts in Section 3 (calibration-marked calls).
At session-89 close it was 0/0/0 — no managers had used the new Good/Bad
popover yet. Goal: by end of Session 90, the team has marked at least 5
calls (mix of good + bad) and the mine script shows real eval-fixture
candidates.

If the count is still 0 after a week of usage, the popover affordance is
too hidden — re-prompt the dev to make it more visible (e.g. surface it
as a primary CTA on low-scored calls).

**Step 2 — Phase 9b drift report against a week of versioned data.**

Run:
```bash
npx tsx scripts/drift-report.ts --days 7
```

By 2026-05-20 there should be a full week of `prompt_version`-stamped
rows. Look for: any surface where avg cost or tok_out moved >20% between
versions. The script auto-flags those with `⚠`. If everything's flat,
the deploy is stable. If something jumped, investigate (was it a prompt
edit, or a tenant-traffic shift?).

**Step 3 — surface watch.**

Two evals were known judge-flaky at session-89 close:
- `full-grading-empty-001` and `full-deal-intel-spanish-001` — single-run
  judge variance. The k=3 majority + one-retry scorer hardening from
  pass 6 should average this out. If they fail in 2+ consecutive weekly
  drift cron runs, apply the Section 28b sharpening pattern (explicit
  VIOLATION + NOT-A-VIOLATION examples in the eval's `mustNotDo` rules).

**Step 4 — when Anthropic ships a new model.**

When `claude-opus-4-7` (or later) drops:
1. `cp evals/reports/full-<latest>.json /tmp/baseline-4-6.json`
2. Search/replace `"claude-opus-4-6"` → `"claude-opus-4-7"` across
   `evals/golden/{smoke,medium,full}.ts` (verify smoke uses inline model
   strings, not constants — there's a sweep there)
3. `EVAL_FORCE=1 npm run evals:full`
4. `npx tsx scripts/model-regression.ts --baseline /tmp/baseline-4-6.json --candidate evals/reports/full-<latest>.json`
5. Review the regression diff before swapping production model IDs.

### What's NOT next

- **More phase work** — LLM Rewiring Plan is shipped. New AI features go
  through the existing `lib/ai/prompts/` + `logAiCall` + eval pattern;
  no further phase scaffolding needed.
- **Drift persistence / dashboard** — gated on having enough versioned
  data to make a dashboard interesting (currently 10 rows). Revisit
  after 2-3 weeks of accrual.
- **`scripts/audit.ts` Anthropic-call wire** — last unwired call site
  (1 row/day dev-facing cron). Low priority; do whenever full
  Anthropic-spend telemetry matters.

### Owner action

None pending. Phase 8 migration ran via Railway auto-deploy. All commits
in `e986fe43..d8a2517b` are live.

---

## Older Next Session content (archived — see `docs/SESSION_ARCHIVE.md`)

The stale Phase 0-5 handoff below was the original Session-87 hand-off
text. Kept for git-blame readability but no longer relevant.
