...
**Owner action items from Session 79 (run when convenient):**
...
1. **Run backfills on Railway** (both idempotent — re-running is safe):
   ...
   npx tsx scripts/backfill-call-source.ts             # apply
...
2. **Bug #11 GHL scope check.** Open the GHL Marketplace App dashboard.
   ...
   listed in the app's scope config. If they're missing, add them and
   reconnect GHL from `Settings → Integrations` (the OAuth re-flow grants
   the new scopes to the existing access token). Test by opening
   `/{tenant}/day-hub` and checking the appointments column — if empty
   list with `error: 'GHL_SCOPE_MISSING'` shows, scopes still aren't
   granted.
3. **Blocker #2 verification ritual.** 6 high-stakes Role Assistant tools
   × approve+reject = 12 trials. Walk-through in
   `docs/AUDIT_PLAN.md` "Blocker #2 verification ritual". Diagnostic
   endpoint: `GET /api/diagnostics/high-stakes-audit?tenant=new-again-houses`.
4. **Phase 1 multi-pipeline live verification** — §12 of
   `docs/plans/ghl-multi-pipeline-bulletproof.md`. 5 tests: live opp
   create, lane isolation, deletion, reverse sync, JV intake.

**Carried forward from Session 78** (still owner-pending):

The buyer architecture wave shipped, the production backfill ran cleanly
on `new-again-houses` (2,055 buyer-info merges + 1,470 "Other" market
strips, 0 errors), but the **GHL custom-field deletion is owner-side
manual work in the GHL UI**. Owner decided Session 79: keep the GHL
fields in place; Gunner buyer page is the team's source of truth, GHL
fields stay as legacy duplicates. **No code action needed — Session 78
