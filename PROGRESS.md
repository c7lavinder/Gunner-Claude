...
**First thing next session (owner-side, ~5 minutes):**
Verify Session 81 once Railway has finished deploying the final commit
(`859ea479`):

1. **Day Hub speed** — open `/{tenant}/day-hub` twice in a row. First
   load is the same speed it was before; second load (within ~5 min)
   should feel noticeably snappier because contact + user lookups are
   cached. If it doesn't feel different, check Railway logs for cache
   errors.
2. **Disposition page** — `/{tenant}/disposition`. Should NOT contain
   any of the 32 dispo-Lost addresses (e.g. 430 Hadleys Bend, 1221
   Wilson Ave SE, 105 William St). Total row count should drop ~16-17
   vs pre-fix.
3. **Inventory "New Deal" chip** — click it. Should show exactly 4:
   3711 Valley View Hwy, 216 Staggs Dr, 2204 Iroquois Pl, 1307 E
   Richton St. NOT the 8 that showed pre-fix.
4. **Spot-check a known-Lost row on inventory.** 6 dispo-Lost
   properties still appear on inventory under "Long-term Follow-up"
   (1031 James White, 1221 Wilson Ave SE, 1221 Wilson, 255 Oak, 413
   Countryside, 4178 Bent Rd) or under their acq UNDER_CONTRACT branch
   (105 William St). This is by design — per-lane semantic confirmed
   with owner 2026-05-11. To fully hide one, mark that lane's GHL opp
   Lost too; the webhook handles the rest within seconds.
5. **Reversibility test (optional)**: pick any of the 32 stamped
   dispo-Lost addresses, reopen its dispo opp in GHL (status → Open).
   Wait ~10 sec, refresh disposition page — the row should reappear.

If any of those break, the `[GHL Webhook]` log lines now print
`oppStatus=...` so it's clear whether GHL is sending the field. Also
re-run `npx tsx scripts/backfill-lost-opps.ts --dry-run` to spot any
new drift.
