...
**No specific carry-forward.** Session 75 closed inventory data quality
end-to-end. Audit on new-again-houses is at 0 actionable findings; the
6 remaining are confirmed false positives (1 fractional address + 5
legit road designations like `Lock 4 Rd` / `U.s. 70 Business`). Going
forward, the parser's 16 newly-supported shapes apply automatically at
GHL ingest time so this regression class shouldn't recur.

**Open candidate work (pick what's most valuable):**
...
6. **Manual verification list** in
   ...
   §12 (live opp test, lane isolation test, deletion test, reverse
   sync test, JV intake test) — owner runs at convenience.
7. **v1.1 reliability scorecard** — pick the next dimension to pull
   from 7 → 9. Item 8 (Seller/Buyer data model) already at 8/10 from
   the v1.1 sprint.

**First prompt to paste into a new Claude session:**

> Read PROGRESS.md Session 75 (closed) to catch up on the inventory
> data-quality work. New diagnostics on disk:
> `scripts/audit-property-addresses.ts` (full audit) and
> `scripts/list-remaining-issues-with-seller.ts` (triage list).
> New mutators include `merge-duplicate-properties.ts` and
> `cleanup-address-shapes.ts --scan-all`. Pick one of the open
> bugs (#16, #18, #22, #24, #25) or move on to v1.1 reliability
> scorecard work.
