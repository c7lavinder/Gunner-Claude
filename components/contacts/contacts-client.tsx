                ...
                <table className="w-full text-[11px]">
                  <thead>
                    <tr className="border-b border-[rgba(0,0,0,0.06)] bg-[#FAFAFA]">
                      ...
                      <th className="text-left px-3 py-2 text-[9px] font-semibold text-gray-500 uppercase tracking-wider">Response</th>
                    ...
                    {filteredBuyers.map(b => {
                      ...
                      return (
                        <tr key={b.id} className="border-b border-[rgba(0,0,0,0.04)] hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="px-3 py-2.5">
                            {b.buyerGrade ? (
                              <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${GRADE_COLORS[b.buyerGrade] ?? 'bg-gray-100 text-gray-600'}`}>
                                {b.buyerGrade}
                              </span>
                            ) : '\u2014'}
                          </td>
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-1">
                              {markets.slice(0, 2).map((m, i) => (
                                <span key={i} className="px-1.5 py-0.5 rounded text-[8px] font-medium bg-blue-50 text-blue-600">
                                  {String(m)}
                                </span>
                              ))}
                              {markets.length > 2 && (
                                <span className="text-[9px] text-gray-400">+{markets.length - 2}</span>
                              )}
                              {markets.length === 0 && <span className="text-gray-300">{'\u2014'}</span>}
                            </div>
                          </td>
                          <td className="px-3 py-2.5 text-gray-700">{b.totalDealsClosedWithUs}</td>
                          <td className="px-3 py-2.5 text-gray-700">{b.blastResponseRate !== null ? `${(b.blastResponseRate * 100).toFixed(0)}%` : '\u2014'}</td>
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-1">
                              ...
                              {!b.isGhost && !b.isVip && b.isActive && <span className="text-gray-300">{'\u2014'}</span>}
                            </div>
                          </td>
                          ...
                          <td className="px-3 py-2.5">
                            {b.ghlContactId ? (
                              <a
                                href={`https://app.gohighlevel.com/contacts/detail/${b.ghlContactId}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-gray-400 hover:text-gray-600"
                              >
