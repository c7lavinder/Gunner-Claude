      ...
      {tab === 'ghl' && (
        <div className="space-y-4">
          <div className="bg-[#1a1d27] border border-white/10 rounded-2xl p-5">
            ...
            {!tenant.ghlConnected && (
              <a
                href={`https://marketplace.gohighlevel.com/oauth/chooselocation?response_type=code&redirect_uri=${encodeURIComponent(window.location.origin + '/api/auth/ghl/callback')}&client_id=${process.env.NEXT_PUBLIC_GHL_CLIENT_ID ?? ''}&scope=contacts.readonly+contacts.write+opportunities.readonly+opportunities.write+conversations.readonly+conversations.write+calls.readonly+tasks.readonly+tasks.write+calendars.readonly`}
